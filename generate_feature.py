...
def create_data_long(bamfile_long_path, outputpath, contig, vcf_file):
    ...
    for ww in contig:
        ...
        for n in range (ider):
            ...
            print(f'chr_name = {chr_name_long},  from {start} to {end}, {n+1}/{ider}')
            ref_pos = []
            loci_clip_sm = []
            loci_clip_ms = []
            ins_count = []
            loci_cover_long ,loci_clip_long_sm, loci_clip_long_ms, loci_ins_long = feature_extraction_long(bamfile_long,chr_name_long,start,end, ref_pos, loci_clip_sm, loci_clip_ms, ins_count)
            ...
            print(f'x_dx_data.shape = {x_data.shape}, index.shape = {index.shape}')
            start = start + 10000000
            end = end + 10000000 
            if len(x_data) == 0:
                continue
            ...
            print(f'x_dx_data.shape = {x_data.shape}, y_label.shape = {y_label.shape}')
...
def create_features_multi_threading(bam_file, output_path, contigs_list, max_worker, vcf_file):
    ...
    while((count) < len(contigs_list)):
            if(len(multiprocessing.active_children()) < int(max_worker)): 
                    ...
                    p = Process(target=create_data_long, args=(bam_file, output_path, [j], vcf_file)) #实例化进程对象
                    p.start()
                    count += 1
            else:
                  time.sleep(2)  